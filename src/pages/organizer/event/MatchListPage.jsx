...
export default function MatchListPage() {
  ...
  const handleLogScore = async (match, sA, sB, umpireName) => {
    ...
    await Promise.all([reloadCategoryData(), loadBracketProgress()]);
  };
  ...
  const handleFinishLive = async (match, sA, sB) => {
    ...
    pushToast('Match recorded', 'success');
    await Promise.all([reloadCategoryData(), loadLiveMatches(), loadBracketProgress()]);
  };
  ...
  return (
    <EventWorkspaceLayout eventName={event?.name}>
      <div className="mb-5">
        ...
        <p className="text-sm text-ink-500">Auto-generate the match schedule for round robin and single elimination categories</p>
      </div>
      ...
      {loggingMatch && (
        <LogScoreModal match={loggingMatch} umpires={umpires} onSave={handleLogScore} onClose={() => setLoggingMatch(null)} />
      )}
      {startingMatch && (
        <StartMatchModal
          match={startingMatch}
